...
If you upload this project to GitHub, do **not** upload your real database username and password.

Keep `db.php` safe. If your database credentials are private, either:

1. Replace them with placeholders before pushing to GitHub, or  
2. Add `db.php` to your `.gitignore` file and provide an example file instead.
...
You can also protect direct access to sensitive PHP files using `.htaccess`.
...
Check the values inside `db.php`:
...
Make sure the command being sent is one of these:
...
### get_state.php returns nothing

Make sure the `robot_state` table exists and contains one row with:
...
```sql
SELECT * FROM robot_state WHERE id = 1;
```

If there is no row, run:
...
If you want to improve this project, pull requests are welcome.
